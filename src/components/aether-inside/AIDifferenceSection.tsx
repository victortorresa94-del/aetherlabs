          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                ...
                <th style={{ ...thBase(''), color: '#00E5FF', fontWeight: 600 }}>
                  Aether Inside
                </th>
              </tr>
            </thead>
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.feature}>
                  ...
                  <td style={{ ...tdCell, backgroundColor: 'rgba(0,229,255,0.03)', borderLeft: '1px solid rgba(0,229,255,0.15)', borderRight: '1px solid rgba(0,229,255,0.15)' }}>
                    <Mark v={row.inside} />
                  </td>
                </tr>
              ))}
...
function Mark({ v }: { v: boolean }) {
  return (
    <span style={{
      fontFamily: 'var(--v5-font-mono)',
      fontSize: '16px',
      color: v ? '#00E5FF' : 'rgba(245,245,245,0.15)',
    }}>
      {v ? '✓' : '✕'}
    </span>
  );
